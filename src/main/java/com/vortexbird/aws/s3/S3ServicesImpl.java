...
public class S3ServicesImpl implements S3Services {
	...
	private final static Logger log=LoggerFactory.getLogger(S3ServicesImpl.class);
	...
	public String uploadPublicRead(String bucketName, String key, File file) throws Exception {
		...
		AmazonS3 amazonS3 = buildAmazonS3(); 
		 try {
			 ...
			 amazonS3.putObject(new PutObjectRequest(bucketName, key,targetStream, new ObjectMetadata()).withCannedAcl(CannedAccessControlList.PublicRead));			 
			 return getUrl(bucketName, key);
		} catch (Exception e) {
			log.error(e.getMessage());
			throw e;
